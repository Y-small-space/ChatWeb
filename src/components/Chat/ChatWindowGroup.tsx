...
import { Avatar, Button, Input, message, Image, Modal, Row, Card, Col } from 'antd';
import {
  ArrowLeftOutlined,
  PictureOutlined,
  TeamOutlined,
...
export const ChatWindowGroup = ({ id, GroupInfo }) => {
  ...
  const renderContent = (message) => {
    ...
    if (message.filename) {
      if (message.filename.toLowerCase().endsWith('.mp4')) {
        return (
  ...
  useEffect(() => {
    api.chat.markGroupMessagesAsRead({ userId, groupId: id })
    getMessage();
    getMessageCurrent();
  }, []);
